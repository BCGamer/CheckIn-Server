                ...
                for varBindTableRow in varBindTable:
                    for name, val in varBindTableRow:
                        ...
                        for oid in mac_matches:
                            full_oid = (
                                dot1dtpfdbport +    # dot1dTpFdbPort - Bridge port
                                oid                 # Add oid from previous result
                            )

                            if str(name) == full_oid:
                                bridge_matches += (val, )


        # Find all interface indexes for this device
        errorIndication, errorStatus, errorIndex, varBindTable = comm.nextCmd(
            snmp_user, snmp_target,
            ifindex
        )
                ...
                for varBindTableRow in varBindTable:
                    for name, val in varBindTableRow:
                        for bridge in bridge_matches:
                            if (ifindex + "." + str(bridge)) == str(name):
                                print('%s = %s' % (name.prettyPrint(), val.prettyPrint()))